...
def autoplay_audio(audio_bytes):
    ...
    st.session_state.currently_playing_audio = audio_bytes
    st.session_state.skip_fast_rerun_until = time.time() + max(2, len(audio_bytes) / 8000 + 1)
    st.session_state.audio_key = f"audio_{time.time()}"
